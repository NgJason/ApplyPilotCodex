...
class CodexBackend(AgentBackend):
    ...
    def build_command(self, *, prompt_path_or_stdin: str, model: str | None, mcp: dict,
        ...
        command = [
            self.cli, "exec", prompt_path_or_stdin, "--json", "--skip-git-repo-check", "--ephemeral",
            # The agent must drive real Chrome over CDP and read generated resume/cover-letter PDFs.
    ...
    def parse_stream(self, line: str) -> AgentEvent | None:
        ...
        text_types = {"agent_message", "agent_message_delta", "assistant_message"}
        if event_type in text_types or (event_type == "item.completed" and item_type in text_types):
            text = _first_present(payload, ("text", "message", "delta", "item.text"))
